...
def masked_diverging_field(X, Y, Z, R, xlabel="x（m）", ylabel="y（m）",
    ...
    cf = ax.contourf(X, Y, Zm, levels=24, cmap=cmap_for("diverging"),
                     norm=norm)
...
def polar_field(theta, r, Z, zlabel="幅值", width="single"):
    ...
    cb = fig.colorbar(pm, ax=ax, shrink=0.75, pad=0.08)
    cb.set_label(zlabel, fontsize=8)
    cb.ax.tick_params(labelsize=7)
    return fig, ax
...
if __name__ == "__main__":
    ...
    fig, ax = masked_diverging_field(
        X, Y, Z, R=150,
        xlabel="口径平面横坐标 ξ₁（m）",
        ylabel="口径平面纵坐标 ξ₂（m）",
        zlabel="促动器径向伸缩量（m）")
    ax.set_title("伸缩量场环状分层，极值出现在 0.62R 环带且均在容差内",
                 fontsize=9, pad=8)
    ...
    fig, ax = polar_field(TH, RR, Zp, zlabel="归一化密度")

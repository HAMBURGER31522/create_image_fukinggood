...
def convergence_curves(curves, xlabel="迭代代数", ylabel="目标函数值",
                       logy=False, conv_tol=1e-3, width="onehalf"):
    """curves: [(名称, best_so_far 数组, 颜色), ...]。
...
if __name__ == "__main__":
    ...
    ga = np.minimum.accumulate(raw1)
    iga = np.minimum.accumulate(raw2)
    fig, ax, info = convergence_curves(
        [("标准 GA", ga, PALETTE[0]), ("改进 GA", iga, "#C97B84")],
        ylabel="总成本（万元）")

...
论点合同示例：
- 结论：Logistic 拟合 R² = 0.999，φ90 = 0.87%，残差全部落在 MC 抽样误差带内。
...
if __name__ == "__main__":
    ...
    fig, _ = fit_residual_pair(
        x, y, xfit, logistic(xfit), resid, band,
        threshold=0.9, x_at_threshold=0.72 + np.log(9) / 14,
        xlabel="介质A 体积分数（%）", ylabel="导通概率",
        fit_label="Logistic 拟合（R² = 0.999）", data_label="蒙特卡洛估计")
    fig.suptitle("Logistic 拟合可信：残差无结构且全部落在抽样误差带内",

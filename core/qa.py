            ...
            for k, (get_lim, get_scale, nm) in enumerate((
                ...
                if used < 0.35 and not _probe.has_field(ax) and not _prop8:
                    ...
                    if float(np.ptp(d)) == 0.0:
                        # 不要报 len(d)：d 是**加密采样后**的点（densify），
                        # 不是用户给的那几个值——实测传 5 个值它会说
                        # "38 个值"。这条检查的全部意义就是别误诊，消息里
                        # 塞个假数字等于自己砸自己的招牌。只报值，不报个数。
                        _hard(f"{nm} 轴上的数据全等于 {float(d[0]):g}（常数"
                              f"列），画成点距图读不出任何差异。把它移出图、"
                              f"放进正文或统计框；确要保留，传 "
                              f"allow=('axis_slack',) 豁免", "axis_slack")
                    else:
